...
def generate_tensors(G, K):
    ...
    sorted_nodes = sorted(eigenvector_centrality.items(), key=lambda item: item[1])

    # Extract node IDs for the least K and highest K nodes
    least_K_nodes = [node for node, _ in sorted_nodes[:K]]
    highest_K_nodes = [node for node, _ in sorted_nodes[-K:]]

    # Initialize tensors
    centrality_tensor = np.full((N,), -1, dtype=int)
    mask_tensor = np.full((N,), False, dtype=bool)

    # Update tensors based on the node positions
    for node_id in range(N):
        if node_id in least_K_nodes:
            centrality_tensor[node_id] = 0
            mask_tensor[node_id] = True
        elif node_id in highest_K_nodes:
            centrality_tensor[node_id] = 1
            mask_tensor[node_id] = True

    return centrality_tensor, mask_tensor
            ...
            if epoch % train_hyperparams["check_loss_freq"] == 0:
                ...
                with torch.no_grad():
                    ...
                    clf.fit(z[datasets['splits'][run_id]['train']],
                            pseudo_labels[datasets['splits'][run_id]['train']])
                    pred = clf.predict(z)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-pseudo_label', '--pseudo', type=int, help='num pseudo labels per class', default=10)
    args = parser.parse_args()
    # Run input parameters
    # General hyperparameters
    hyper_parameters = {'train_perc': args.train, 'val_perc': args.val, 'test_perc': args.test,
                        'tsim_th': args.tsim_th, 'val_metric': args.val_metric, 'undersampling': args.under,
