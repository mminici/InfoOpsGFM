...
def read_all_data(device_id, dataset_name, hyper_params, train_hyperparams, model_hyperparams):
    ...
    datasets = create_data_loader(data_dir, hyper_params['tsim_th'],
                                  hyper_params['train_perc'],
                                  hyper_params['undersampling_perc'], hyper_params['undersampling'])
    ...
    return device, base_dir, interim_data_dir, data_dir, datasets, edge_index, network, struct_node_features
...
def main(dataset_name, train_hyperparams, model_hyperparams, hyper_params, device_id):
    ...
    device, base_dir, interim_data_dir, data_dir, datasets, edge_index, network, struct_node_features = read_all_data(device_id, dataset_name, hyper_params, train_hyperparams, model_hyperparams)
    model_hyperparams['feature_dim'] = struct_node_features.shape[1]
    # Create loggers
    ...
    for country in other_countries:
        other_countries_hyperparams = copy.deepcopy(hyper_parameters)
        other_countries_hyperparams['undersampling'] = None
        other_countries_hyperparams['undersampling_perc'] = None
        _, _, _, _, country_datasets, country_edge_index, country_network, country_struct_node_features = read_all_data(
            device_id, country, other_countries_hyperparams, train_hyperparams, model_hyperparams)
        countries_data[country] = {'datasets': country_datasets, 'edge_index': country_edge_index,
                                   'network': country_network, 'struct_node_features': country_struct_node_features}
    ...
    for run_id in tqdm(range(hyper_params['num_splits']), 'Splits training'):
        ...
        for epoch in range(num_epochs):
            ...
            model.train()
            optimizer.zero_grad()
            countries_pred = {country: model(countries_data[country]['struct_node_features'],
            ...
            if epoch % train_hyperparams["check_loss_freq"] == 0:
                ...
                model.eval()
                with torch.no_grad():
                    pred = model(struct_node_features, edge_index).detach().cpu().numpy().flatten()
        ...
        metric_to_optimize = train_hyperparams['metric_to_optimize']
        BEST_VAL_METRIC = -np.inf
        best_model_path = interim_data_dir / f'FT_model{run_id}.pth'
        # Create optimizer
        optimizer = torch.optim.Adam(model.parameters(), lr=train_hyperparams['learning_rate'] * 1e-1)
        loss_fn = torch.nn.BCELoss()
        early_stopping_cnt = 0
        for epoch in range(num_epochs):
            if early_stopping_cnt > train_hyperparams["early_stopping_limit"]:
            ...
            model.train()
            optimizer.zero_grad()
            pred = model(struct_node_features, edge_index).flatten()
