...
DEFAULT_HYPERPARAMETERS = {'train_perc': 0.7,
                           ...
                           'is_few_shot': False,
                           'num_splits': 20}
DEFAULT_TRAIN_HYPERPARAMETERS = {'input_embed': 'positional_degree', 'epochs': 1000, 'learning_rate': 1e-3,
...
def create_model(model_hyperparameters):
    return MLP(in_dim=model_hyperparameters['feature_dim'], hidden_dim=model_hyperparameters['latent_dim'],
               dropout_p=model_hyperparameters['dropout'])
...
def run_experiment(dataset_name='cuba',
    ...
    for run_id in range(num_splits):
        ...
        for epoch in range(num_epochs):
            ...
            model.train()
            optimizer.zero_grad()
            pred = model(node_features).flatten()
            ...
            if epoch % train_hyperparameters["check_loss_freq"] == 0:
                ...
                model.eval()
                with torch.no_grad():
                    pred = model(node_features).detach().cpu().numpy().flatten()
...
if __name__ == '__main__':
    ...
    train_hyper_parameters = {'input_embed': 'positional_rw', 'num_epochs': 1000, 'learning_rate': 1e-2,
                              'early_stopping_limit': 10, 'check_loss_freq': 5, 'metric_to_optimize': 'f1_macro'}
    # model hyperparameters
    latent_dim = 100
    model_hyper_parameters = {'latent_dim': latent_dim, 'dropout': 0.2}
    for seed_val in seed:
        mlflow.set_experiment(f'{dataset_name}-MLP-{seed_val}')
