...
def generate_tensors(G, K):
    ...
    # Initialize tensors
    centrality_tensor = torch.full((N,), -1., dtype=torch.float)
    mask_tensor = torch.full((N,), False, dtype=torch.bool)

    # Update tensors based on the node positions
    for node_id in range(N):
        if node_id in least_K_nodes:
            centrality_tensor[node_id] = 0.0
            mask_tensor[node_id] = True
        elif node_id in highest_K_nodes:
            centrality_tensor[node_id] = 1.0
            mask_tensor[node_id] = True
