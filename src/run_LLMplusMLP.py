...
def main(dataset_name, train_hyperparams, model_hyperparams, hyper_params, device_id):
    ...
    datasets = create_data_loader(data_dir, hyper_params['tsim_th'], tr_perc=hyper_params['train_perc'],
                                  undersampling=hyper_params['undersampling'])
    ...
    for run_id in tqdm(range(hyper_params['num_splits']), 'Splits training'):
        ...
        class TweetClassifier(nn.Module):
            def __init__(self, num_layers, input_neurons, hidden_neurons, output_neurons, dropout_percentage=0.2,
        ...
        # Create the model
        model = TweetClassifier(num_layers=model_hyperparams['num_layers'],
                                input_neurons=768,  # Sentence-Transformer output dimension
                                hidden_neurons=model_hyperparams['latent'],
                                output_neurons=1,
                                dropout_percentage=model_hyperparams['dropout'])
        model.to(device)
        # Create optimizer
...
if __name__ == '__main__':
    ...
    parser.add_argument('-latent_dim', '--latent', type=int, help='Dimension of the hidden layers', default=128)
    parser.add_argument('-dropout', '--dropout', type=float, help='Dropout percentage', default=0.2)
    parser.add_argument('-num_layers', '--num_layers', type=int, help='Num hidden layers', default=3)
    ...
    model_hyperparameters = {'latent': args.latent, 'dropout': args.dropout, 'num_layers': args.num_layers}
